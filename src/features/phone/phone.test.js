...
describe('PHONE', () => {
  describe('Validation', () => {
    ...
    it('should invalidate a fake phone', () => {
      invalidPhones.map(invalidPhone => assert(PHONE.validate(invalidPhone) == false))
    })
  ...
  describe('Formatting', () => {
    ...
    it('should format a phone', () => {
      validPhones.map(({ raw, formatted }) => assert(PHONE.format(raw) == formatted))
    })
